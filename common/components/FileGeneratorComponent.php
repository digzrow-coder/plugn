...
class FileGeneratorComponent extends Component
{
    ...
    {
        $apiEndpointLiteral = json_encode((string) $apiEndpoint, JSON_UNESCAPED_SLASHES);

        if ($apiEndpointLiteral === false) {
            throw new InvalidConfigException('Unable to encode API endpoint for generated build.js');
        }
        ...
        $buildJsContent = <<<'JS'
                ...
                var apiEndPoint = normalizeApiEndpoint(__PLUGN_API_ENDPOINT__);

                var storebranchName = validateStoreBranchName(process.env.PLUGN_STORE_BRANCH || process.env.BRANCH || 'main');
...
JS;
        ...
        return str_replace('__PLUGN_API_ENDPOINT__', $apiEndpointLiteral, $buildJsContent);
    }
